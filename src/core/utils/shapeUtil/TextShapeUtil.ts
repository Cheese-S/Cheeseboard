import { Shape, Bound, Point, Rect, Text } from "../../type";
...
export default class TextShapeUtil extends ShapeUtil {
    ...
    get_bound(shape: Text, rotated?: boolean): Bound {
        return get_rect_bound(shape, rotated);
    }

    intersect_bound(bd: Bound, shape: Text): boolean {
    ...
    rot_shape_about(p: Point, r: number, shape: Text): void {
        shape.center = Vec.rot_about(shape.center, p, r);
        shape.r = (shape.r + r);
    }

    translate_shape(delta: Point, shape: Text): void {
        shape.center = Vec.add(shape.center, delta);
    }

    set_shape_top_left(shape: Text, pt: Point): void {
